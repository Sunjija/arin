...
제출 `c4feb4ea4f06deea2bf1b1c52ffd87f4078f2215`를 `db8823f`로 반입했다. 앞선 절은 Cursor 제출 당시 범위이며, 이후 총괄은 다음을 보완했다.

- 결과의 `correct/total`을 `정답 N / M문제`로 명확하게 표시하고 전체 결과 합산도 중복 questionId를 제거해 그룹 결과와 일치시켰다.
- recent-wrong fixture가 이전 답안 0회였던 모순을 수정했다. 날짜 비교 타입과 구형 마이그레이션 fixture도 총괄이 수정했다.
- 실제 브라우저에서 카드 0장·복습 문제 1개인데 시작할 수 없는 문제를 발견해 CardsPage와 진입 테스트를 보완했다. 이전 날짜의 미완료 학습도 이어간다.
- 복습 결과를 새로고침하면 자동으로 다음 회차가 생성되는 문제를 고쳤다. 새 회차는 목록의 명시적 시작만 허용하며, 재개 API와 테스트를 함께 보완했다.
- 전체 **239개 / 49파일**, 타입·전체 lint·프로덕션 빌드 통과. 320/375/768px·데스크톱 및 키보드에서 새 문구와 주요 흐름을 확인했다. 백업·자정·저장 실패는 자동 테스트로 검증했다.

실행 범위와 미검증 항목은 [통합 검증](../p1-review-validation.md)에 기록한다. 200% 글자 확대·전 범위 역사 검수는 남아 있고 공개 배포는 Sites 접근 오류로 미완료다. 제출한 UI의 품질과 서비스 전체 흐름의 품질은 별도로 검수해야 한다.
