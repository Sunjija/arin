...
P0-01 코드 검토 후 부분 통합을 진행했다. [PR 검토](../pr-integration-review.md), [콘텐츠 공백](../content-coverage.md), [현재 API/남은 게이트](e-foundation-api.md), [검증 기록](../p0-validation.md)을 우선 확인한다. [Cursor 콘텐츠 작업 1건을 실제 배정·인수했다](../cursor-tasks/README.md). 추가 작업은 미배정이다. 아래 과거 감사 SHA는 이번 통합의 전체 동작 기준이 아니다.
...
현재 통합 브랜치에는 단원 18, 내부 개념 88, 자체 문항 102, 카드 95, 연표 사건 70이 있다. 설명 7개·명시적 연결 문항 8개를 보강했다. 새 일일 학습은 개념 → 확인 문제 → 누적 복습이며 목표일·요일에 따른 개념 분량을 적용한다. 구형 미완료 세션은 원래 목록을 유지한다. 상세 설명 81개와 자료실 단계 재개·실전 검수는 남아 있다. main·공개 배포 상태는 별도다.

## 읽기 순서와 작업 순서
`../../AGENTS.md` → [전체 개발 로드맵](../development-roadmap.md) → [정책·계약](contracts.md) → [소유권](ownership.md) → 담당 brief.
전체 우선순위·의존성·완료 기준은 로드맵이 원본이다. 다음 후보는 P1-04 자료실 재개와 P1-03 복습 이유·노출 구분이며 아래 순서나 역할 표는 배정 완료를 뜻하지 않는다.
