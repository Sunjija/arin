...
## 2026-09-13: 준비된 콘텐츠로 자료실 확대

`preparedLibraryQuestions(lessonId)` (`src/lib/libraryQuestionBank.ts`)는 초안이 아닌 가이드에서 설명·출처가 있는 개념에 전부 명시 연결되고 문항 출처가 있는 질문을 반환한다. 신규 세션 생성과 재시작, UI 문항 수가 같은 함수를 사용한다. 소스 존재 조건은 사람 승인·실전 품질 검증이 아니다.

`startLibraryPractice`는 기존 유효 세션이 있으면 필터 변화와 무관하게 저장 스냅샷을 돌려준다. 새로 생성할 문항이 없으면 준비 중 오류가 나고, 기존 진행/답안을 지우지 않는다. 가이드가 초안으로 바뀌거나 현재 문항이 없어져도 이미 저장된 진행은 보존된다. DB/백업 버전과 함수 인자는 그대로다.

자료실은 모든 단원에 진행 조회 UI를 연결한다. 새 문항이 없고 저장된 진행도 없으면 준비 중 안내, 있으면 재개한다. 현재 준비 문항은 lesson-01 10개, lesson-02 8개, lesson-13 4개. 단원 선택/읽기 스크롤 위치 자체의 복원은 후속 범위다. [검증 보고서](../reviews/p2-silla-content.md#8-총괄-독립-검토최종-통합-2026-09-13).
