...
export function hydrateConceptProgress(input: {
  ...
  for (const concept of concepts) {
    ...
    if (concept.lessonId && doneLessons.has(concept.lessonId) && row.learnState === 'unseen') {
      const completion = input.completions.find((item) => item.lessonId === concept.lessonId)
      // Legacy lesson completion cannot prove every catalog concept was taught.
      row.learnState = 'learning'
  ...
  for (const [questionId, list] of attemptsByQuestion) {
    ...
    const conceptsFor = conceptsForQuestion(
      {
        ...
        era: latest.era,
        conceptIds: latest.snapshot?.conceptIds ?? (latest.conceptId ? [latest.conceptId] : []),
      },
      concepts,
    )
    for (const concept of conceptsFor) {
      ...
      if (!row.lastAttemptAt || row.lastAttemptAt < latest.createdAt) {
        row.lastAttemptAt = latest.createdAt
        row.lastAttemptId = latest.id
      }
      // Exposure-aware calibration belongs to P2; answer frequency is not mastery.
      byId.set(concept.id, row)
    }
  }
