...
export function selectStudyQuestions(ctx: SelectionContext): SplitSelection {
  ...
  const reviewPool = ctx.questions.filter((question) => {
    if (exclude.has(question.id)) return false
    if (ctx.todayLessonId && question.lessonId === ctx.todayLessonId) return false
  ...
  if (!split) {
    return { newQuestions, reviewQuestions: [], all: fillLegacyMix(ctx, newPool, used, boostTypes, boostMultiplier) }
  }
