...
describe('progress evidence summary', () => {
  it('counts completed catalog concepts only and preserves honest unprepared coverage', () => {
    ...
    expect(schedule).toMatchObject({ totalConcepts: 88, completedConcepts: 1, remainingConcepts: 87, readyRemainingConcepts: 6, unavailableConcepts: 81, allContentReadyFinishDate: null })
  ...
  it('retains scheduler warnings for expired dates, rest days, and unavailable explanations', () => {
    ...
    expect(schedule.warnings.join(' ')).toContain('상세 설명이 준비되지 않은 개념이 81개')
