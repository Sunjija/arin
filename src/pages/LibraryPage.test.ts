...
describe('LibraryPage', () => {
  ...
  it('shows concept summary, keywords, and checkpoints without study completion copy', async () => {
    ...
    expect(lesson).toHaveAttribute('aria-current', 'true')
    const reader = screen.getByRole('article', { name: '선사 문화와 고조선' })
    expect(within(reader).getByText(/고조선의 건국 전통과/)).toBeTruthy()
    expect(within(reader).getByText(/비파형동검, 고인돌, 군장 사회/)).toBeTruthy()
    expect(within(reader).getByText('기억할 핵심어')).toBeTruthy()
    expect(screen.queryByText('오늘 학습으로')).toBeNull()
