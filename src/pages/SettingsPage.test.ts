...
describe('SettingsPage save validation', () => {
  it('rejects 0 daily questions and does not write the invalid value', async () => {
    ...
    expect((await screen.findAllByText('하루 문제 수는 5에서 40 사이의 정수로 입력하세요.')).length).toBeGreaterThan(0)
    expect(screen.queryByText(/목표를 저장했습니다/)).toBeNull()
  ...
  it('saves the allowed boundary of 5 questions', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '설정 저장' }))
    expect(await screen.findByText(/목표를 저장했습니다/)).toBeTruthy()
